 ...
 * - Input and origin messages have different byte alignments for the stick and trigger values.
 */
...
/**
 * WaveBird input state message struct "view" for convenience.
 */
struct wavebird_input_state {
  uint8_t header_and_buttons[4];
  uint8_t stick_x;
  uint8_t stick_y;
  uint8_t substick_x;
  uint8_t substick_y;
...
/**
 * WaveBird input state message button bitfield struct.
 */
struct wavebird_input_state_buttons {
  uint16_t left  : 1;
  uint16_t right : 1;
  uint16_t down  : 1;
  uint16_t up    : 1;
...
/**
 * Copy the origin data from a WaveBird origin message into a buffer.
 *
 * @param dest pointer to a 6-byte buffer to store the origin data
 * @param message the message decoded from a WaveBird packet
 */
static inline void wavebird_origin_copy(uint8_t *dest, const uint8_t *message)
{
  dest[0] = (message[2] & 0x0F) << 4 | message[3] >> 4;
  dest[1] = (message[3] & 0x0F) << 4 | message[4] >> 4;
  dest[2] = (message[4] & 0x0F) << 4 | message[5] >> 4;
  dest[3] = (message[5] & 0x0F) << 4 | message[6] >> 4;
  dest[4] = (message[6] & 0x0F) << 4 | message[7] >> 4;
  dest[5] = (message[7] & 0x0F) << 4 | message[8] >> 4;
}